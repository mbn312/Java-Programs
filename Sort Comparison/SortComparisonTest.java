...
public class SortComparisonTest {
	...
	public void testPartition() {
		String[] words = createRandomStrings(1024, 5); // 10);
		String[] expectedWords = words.clone();
	...
	public void testMerge() {
		String[] originalWords = new String[] { "M", "B", "Z", "A", "F", "D", "C", "P", "Q", "E", "V", "X" };

		String[] words = new String[] { "M", "B", "Z", "A", "F", "D", "C", "P", "Q", "E", "V", "X" };

		String[] expectedWords = ArrayUtils.subarray(words, 6, 12);
		Arrays.sort(expectedWords);
	...
	public void testHeapify() {
		String[] words = createRandomStrings(100000, 5); // 10);
	...
	void standardSortTest(int sortId, int wordLength, int minN, int maxN, int stepN,
		...
		for (int count = minN; count <= maxN; count += stepN) {
			...
			for (int ndx = 0; ndx < numSamplesPerTest; ndx++) {
				...
				if (dataOrder == DUPLICATE_DATA) {
					String word = RandomStringUtils.randomAlphabetic(wordLength)
							.toUpperCase();
		...
		if (printResults) {
			System.out.println("\tN = " + wordCounts[0] + ", time = " + String.format("%.3f sec", elapsedTimes[0]));
			System.out.println("\tN = " + wordCounts[elapsedTimes.length - 1] + ", time = "
					+ String.format("%.3f sec", elapsedTimes[elapsedTimes.length - 1]));
			System.out.println("\tCount:  " + countRatio + "x,  Time: " + String.format("%.1f", timeRatio) + "x");
		}
